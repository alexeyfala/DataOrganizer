...
- ⚙️ [`app.manifest`](../DataOrganizer.Desktop/app.manifest) — манифест Windows: список поддерживаемых версий системы; без него ломаются прозрачность окна и встроенные элементы. **Когда:** почти никогда.
